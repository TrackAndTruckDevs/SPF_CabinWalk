/**
 * @file SPF_CabinWalk.hpp
 * @brief Internal header for the SPF_CabinWalk.
...
struct PluginContext {
  ...
  SPF_TruckData last_truck_data = {0};  // Cache for last known truck data
    ...
    * @details This is defined once in `SPF_CabinWalk.cpp` and declared `extern` here, making it
 ...
 */
void LoadSettings(const SPF_Config_API* configAPI, SPF_Config_Handle* h);
...
/**
 * @brief Called when the game world is being unloaded.
 * @details Use this to reset world-scoped state (e.g., mark hook as unregistered).
 */
 ...
 * @details This is for the "move to passenger seat" action.
 */
void OnMoveToPassengerSeat();

/**
 * @brief Callback executed when a keybind action is triggered by the user.
 * @details This is for the "move to driver seat" action.
 */
