...
AnimationSequence::AnimationSequence() : m_duration_ms(0), m_is_playing(false), m_current_elapsed_time_ms(0) {
...
bool AnimationSequence::Update(uint64_t delta_time_ms, const SPF_Camera_API* camera_api) {
  ...
  const float current_progress = (m_duration_ms == 0) ? 1.0f : static_cast<float>(m_current_elapsed_time_ms) / static_cast<float>(m_duration_ms);
