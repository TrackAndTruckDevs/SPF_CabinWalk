...
static std::map<std::pair<CameraPosition, CameraPosition>, std::function<std::unique_ptr<Animation::AnimationSequence>(const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state)> > g_sequence_factory;
...
void NotifySettingsUpdated() { g_settings_dirty = true; }
...
void Initialize(PluginContext* ctx) {
  ...
  RegisterSequence(CameraPosition::Driver, CameraPosition::Passenger, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) {
    return AnimationSequences::CreateDriverToPassengerSequence(start_state, target_state);
  });

  RegisterSequence(CameraPosition::Passenger, CameraPosition::Driver, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) {
    return AnimationSequences::CreatePassengerToDriverSequence(start_state, target_state);
  });

  // --- Driver <-> Standing Sequences ---
  RegisterSequence(
    CameraPosition::Driver, CameraPosition::Standing, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) { return AnimationSequences::CreateDriverToStandingSequence(start_state, target_state); });

  RegisterSequence(
    CameraPosition::Standing, CameraPosition::Driver, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) { return AnimationSequences::CreateStandingToDriverSequence(start_state, target_state); });

  // --- Passenger <-> Standing Sequences ---
  RegisterSequence(CameraPosition::Passenger, CameraPosition::Standing, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) {
    return AnimationSequences::CreatePassengerToStandingSequence(start_state, target_state);
  });

  RegisterSequence(CameraPosition::Standing, CameraPosition::Passenger, [](const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) {
...
void MoveTo(CameraPosition target) {
  ...
  if (it != g_sequence_factory.end()) {
    ...
    if (target == CameraPosition::Driver) {
      if (g_anim_ctx->cameraAPI) {
        g_anim_ctx->cameraAPI->Cam_SetInteriorSeatPos(g_cached_driver_state.position.x, g_cached_driver_state.position.y, g_cached_driver_state.position.z);
        g_anim_ctx->cameraAPI->Cam_SetInteriorHeadRot(g_cached_driver_state.rotation.x, g_cached_driver_state.rotation.y);
      }
    } else {
      const auto& target_transform = GetTargetTransformForPosition(target);
      if (g_anim_ctx->cameraAPI) {
...
CameraPosition GetCurrentPosition() { return g_current_pos; }
...
void OnRequestMove(CameraPosition final_destination) {
  ...
  if ((current_pos == CameraPosition::Driver && final_destination == CameraPosition::Passenger) || (current_pos == CameraPosition::Passenger && final_destination == CameraPosition::Driver)) {
...
void QueueMove(CameraPosition target) { g_pending_moves.push(target); }
...
void Reset() {
  g_current_pos = CameraPosition::Driver;
  g_target_pos = CameraPosition::Driver;
  g_active_sequence = nullptr;
  ClearPendingMoves();
}

bool HasPendingMoves() { return !g_pending_moves.empty(); }

void RegisterSequence(CameraPosition from, CameraPosition to, std::function<std::unique_ptr<Animation::AnimationSequence>(const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state)> factory) {
