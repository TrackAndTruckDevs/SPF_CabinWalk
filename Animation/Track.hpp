...
class Track {
 ...
 public:
  ...
  void AddKeyframe(const Keyframe<T>& keyframe) {
    ...
    std::sort(m_keyframes.begin(), m_keyframes.end(), [](const Keyframe<T>& a, const Keyframe<T>& b) { return a.progress < b.progress; });
  }
  ...
  T Evaluate(float current_progress, T default_value) const {
    ...
    auto it_end = std::upper_bound(m_keyframes.begin(), m_keyframes.end(), current_progress, [](float prog, const Keyframe<T>& kf) { return prog < kf.progress; });
 ...
 private:
  ...
  T lerp(const T& a, const T& b, float t) const { return a + (b - a) * t; }
};
