...
std::unique_ptr<Animation::AnimationSequence> CreateSofaToStandingSequence(const Animation::CurrentCameraState& start_state, const Animation::CurrentCameraState& target_state) {
  auto seq = std::make_unique<Animation::AnimationSequence>();
  seq->Initialize(g_ctx.settings.animation_durations.main_animation_speed.sofa_to_standing * 1000);
