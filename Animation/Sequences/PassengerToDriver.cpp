...
namespace SPF_CabinWalk::AnimationSequences
{
    ...
    )
    {
        ...
        const uint64_t total_duration_ms = Animation::Config::PASSENGER_TO_DRIVER_DURATION_MS;
        seq->Initialize(total_duration_ms);
        ...
        // --- Position Y Track (Move Up/Down) ---
        {
            ...
            track->AddKeyframe({0.55f, 0.255f, Easing::easeInQuint});
            track->AddKeyframe({0.85f, 0.250f, Easing::linear});
        ...
        // --- Rotation Yaw Track (Look Left/Right) ---
        {
            ...
            track->AddKeyframe({0.2f, 1.35f, Easing::easeOutCubic});
            //track->AddKeyframe({0.55f, 0.85f, Easing::linear});
            track->AddKeyframe({0.65f, 0.15f, Easing::linear});
            // track->AddKeyframe({0.75f, 0.0f, Easing::easeOutQuad});
            //track->AddKeyframe({0.95f, -0.15f, Easing::easeOutCubic});
