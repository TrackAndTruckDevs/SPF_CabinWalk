...
std::unique_ptr<Animation::AnimationSequence> CreateDynamicFirstStepSequence(const Animation::CurrentCameraState& initial_state, bool is_walking_forward) {
  ...
  float target_yaw = is_walking_forward ? 0.0f : std::numbers::pi;  // Start with +180
  ...
  if (!is_walking_forward) {
    ...
    if (current_yaw < 0) {
      target_yaw = -std::numbers::pi;
    }
  }
  float angle_to_turn = std::fabs(current_yaw - target_yaw);

  // Handle wrapping for angle_to_turn
  if (angle_to_turn > std::numbers::pi) {
    angle_to_turn = 2 * std::numbers::pi - angle_to_turn;
  }
  ...
  uint64_t turn_duration_ms = base_turn_duration_ms + static_cast<uint64_t>((angle_to_turn / std::numbers::pi) * extra_turn_duration_ms_per_pi);
